...
5. Если нужен **локальный прокси** (Clash, v2rayN и т.п.) — пропиши в `.env`: `TELEGRAM_PROXY=http://127.0.0.1:ПОРТ` (порт возьми из настроек клиента; для **SOCKS5** зависимость уже в `requirements.txt` через `[socks]`).

Скрипт **`run_bot.ps1`** перед стартом бота сам запускает `check_telegram.py`. Запуск **напрямую** `python bot.py` проверку не делает, но бот при старте будет **бесконечно повторять** подключение (`bootstrap_retries=-1`), пока сеть не появится — остановка **Ctrl+C**.
